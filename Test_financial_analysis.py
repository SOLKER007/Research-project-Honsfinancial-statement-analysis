...
def extract_candidate_tables(pdf, target_pages, lookahead=2):
    all_tables = []
    considered = set()
...
# -----------------------
# 4) Pull it all together (extract -> classify -> pick best)
# -----------------------
with pdfplumber.open(pdf_path) as pdf:
    pages = find_income_pages(pdf)
    if not pages:
        raise ValueError("Could not locate the income statement heading in the PDF.")
    print("🎯 Income statement likely on/near pages:", [p+1 for p in pages])
    candidates = extract_candidate_tables(pdf, pages, lookahead=2)

if not candidates:
    raise RuntimeError("No usable tables extracted near the income statement pages.")
...
for _, _, cand in scored:
    ...
    lab_textiness = pd.Series(cleaned.index.astype(str)).str.contains(r"[A-Za-z]", regex=True).mean() if cleaned is not None and not cleaned.empty else 0.0
    if cleaned.shape[0] >= 2 and cleaned.shape[1] >= 1 and lab_textiness >= 0.15:
        best_df = cleaned
        break

# -----------------------
# 5) Row picking (robust normalizing matcher: index OR any cell)
# -----------------------
def _norm(s: str) -> str:
...
revenue_cues = ["revenue", "turnover", "group turnover", "external turnover"]
...
def normalize_number(txt: str) -> float | None:
    ...
    # Percent values are not "amounts"
    if "%" in txt:
        return None

    t = txt.strip()

    # Normalize various minus signs to ASCII hyphen
    t = t.replace("\u2212", "-")   # Unicode minus → '-'
    t = t.replace("−", "-")        # another variant

    # Detect accounting negative with parentheses
    neg = ("(" in t) and (")" in t)

    # Remove parentheses now that we've recorded negativity
    t = t.replace("(", "").replace(")", "")

    # Remove spaces, thin/non-breaking spaces, and grouping commas
    t = (t
         .replace("\u2009", "")    # thin space
    ...
    # Strip currency and other non-numeric, keeping digits, dot, minus
    t = re.sub(r"[^0-9.\-]", "", t)
...
def numeric_segments(ws, gap_tol=8.0):
    ...
    def is_num_token(t: str) -> bool:
        ...
        if re.search(r"[A-Za-z]", t):
            return False
        # allow (), digits, separators, dot, minus, and common spaces
...
def find_metrics_from_words_two_periods(pdf, target_pages):
    ...
    for p in target_pages:
        ...
        for _, ws, line_text in read_lines(page):
            ...
            if not segs:
                continue
            segs_sorted = sorted([seg for seg in segs if abs(seg[0]) >= 1000] or segs, key=lambda t: t[1])
...
if margins is None or margins.empty:
    for k, (_, _, cand) in enumerate(scored[:5], 1):
        try:
            ...
            pass
    raise ValueError("Could not compute margins from tables or page text.")
...
# --------------------------------------------------------
# 8) Visualisations (show live; add linear regression + extras)
# --------------------------------------------------------
# - matplotlib only
...
def as_period_axis(idx_like):
    """
    Convert amounts/margins index (often years as strings) to numeric x.
    If a year is found, use it; else use a 0..N-1 sequence.
    Returns x_numeric (list[float]), labels (list[str]).
    """
    ...
    if any(y is not None for y in years):
        # Fill missing with previous+1 to keep monotonic x
        x = []
        cur = None
        for y in years:
            if y is not None:
...
def fit_linreg(x, y):
    """
    Simple OLS line y = a*x + b with numpy.polyfit (degree 1).
    Returns (a, b, r2). Only if >=2 non-na points.
    """
...
def plot_line_with_regression(x, y, labels, title, ylab):
    """Line plot across periods with regression overlay."""
    if len(x) < 1:
        return
    fig = plt.figure()
...
def plot_scatter_with_regression(x, y, title, xlab, ylab):
    """Scatter with regression line (y on x)."""
    if len(x) < 2:
        return
    fig = plt.figure()
    plt.scatter(x, y)
...
# ---------- Existing charts ----------
# A) Margins chart (grouped bars by metric across periods)
if margins is not None and not margins.empty:
    fig = plt.figure()
    periods = list(margins.index.astype(str))
    metrics_cols = list(margins.columns)
    x = range(len(periods))
    width = 0.8 / max(1, len(metrics_cols))
    for i, col in enumerate(metrics_cols):
        y = [float(margins.loc[p, col]) if pd.notna(margins.loc[p, col]) else 0 for p in periods]
        offsets = [xi + (i - (len(metrics_cols)-1)/2)*width for xi in x]
        plt.bar(offsets, y, width=width, label=col)
    plt.xticks(list(x), periods)
    plt.ylabel("Margin (ratio)")
    plt.title("Profitability Margins by Period")
    plt.legend()
    show_fig(fig)

# B) Revenue vs Net Income amounts (grouped bars)
if amounts is not None and {"Revenue", "Net Income"}.issubset(amounts.columns):
    fig = plt.figure()
    idx = list(amounts.index.astype(str))
    width = 0.35
    x = range(len(idx))
    rev = [float(amounts.loc[i, "Revenue"]) if pd.notna(amounts.loc[i, "Revenue"]) else 0 for i in idx]
    ni  = [float(amounts.loc[i, "Net Income"]) if pd.notna(amounts.loc[i, "Net Income"]) else 0 for i in idx]
    plt.bar([xi - width/2 for xi in x], rev, width=width, label="Revenue")
    plt.bar([xi + width/2 for xi in x], ni,  width=width, label="Net Income")
    plt.xticks(list(x), idx)
    plt.ylabel("Amount (Rm)")
    plt.title("Revenue vs Net Income")
    plt.legend()
    show_fig(fig)

# C) YoY Growth chart (if we have Latest & Prior)
# (Shown as bar; could also add regression if more than 2 periods exist)
if "YoY Growth" in extras and isinstance(extras["YoY Growth"], pd.Series) and not extras["YoY Growth"].empty:
    fig = plt.figure()
    g = extras["YoY Growth"].sort_index()
    plt.bar(range(len(g)), list(g.values))
    plt.xticks(range(len(g)), list(g.index), rotation=15, ha="right")
    plt.ylabel("YoY Growth (ratio)")
    plt.title("Year-over-Year Growth")
    show_fig(fig)

# ---------- NEW: Line charts with regression ----------
if amounts is not None and "Revenue" in amounts.columns:
    x_num, lbls = as_period_axis(amounts.index)
    y_rev = [float(v) if pd.notna(v) else np.nan for v in amounts["Revenue"]]
    plot_line_with_regression(x_num, y_rev, lbls, "Revenue Trend (with Linear Regression)", "Revenue (Rm)")

if amounts is not None and "Net Income" in amounts.columns:
    x_num, lbls = as_period_axis(amounts.index)
    y_ni = [float(v) if pd.notna(v) else np.nan for v in amounts["Net Income"]]
    plot_line_with_regression(x_num, y_ni, lbls, "Net Income Trend (with Linear Regression)", "Net Income (Rm)")

if amounts is not None and "Operating Profit" in amounts.columns:
    x_num, lbls = as_period_axis(amounts.index)
    y_op = [float(v) if pd.notna(v) else np.nan for v in amounts["Operating Profit"]]
    plot_line_with_regression(x_num, y_op, lbls, "Operating Profit Trend (with Linear Regression)", "Operating Profit (Rm)")

if margins is not None and "Gross Margin" in margins.columns:
    x_num, lbls = as_period_axis(margins.index)
    y_gm = [float(v) if pd.notna(v) else np.nan for v in margins["Gross Margin"]]
    plot_line_with_regression(x_num, y_gm, lbls, "Gross Margin Trend (with Linear Regression)", "Gross Margin (ratio)")

if margins is not None and "Operating Margin" in margins.columns:
    x_num, lbls = as_period_axis(margins.index)
    y_om = [float(v) if pd.notna(v) else np.nan for v in margins["Operating Margin"]]
    plot_line_with_regression(x_num, y_om, lbls, "Operating Margin Trend (with Linear Regression)", "Operating Margin (ratio)")

# ---------- NEW: Scatter relationships with regression ----------
if amounts is not None and {"Revenue", "Net Income"}.issubset(amounts.columns):
    x = [float(v) for v in amounts["Revenue"] if pd.notna(v)]
    y = [float(v) for v in amounts["Net Income"] if pd.notna(v)]
    n = min(len(x), len(y))
    plot_scatter_with_regression(x[:n], y[:n], "Net Income vs Revenue (with Linear Fit)", "Revenue (Rm)", "Net Income (Rm)")

if amounts is not None and {"Revenue", "Operating Profit"}.issubset(amounts.columns):
    x = [float(v) for v in amounts["Revenue"] if pd.notna(v)]
    y = [float(v) for v in amounts["Operating Profit"] if pd.notna(v)]
    n = min(len(x), len(y))
    plot_scatter_with_regression(x[:n], y[:n], "Operating Profit vs Revenue (with Linear Fit)", "Revenue (Rm)", "Operating Profit (Rm)")

# ---------- NEW: Conversion & Spread lines (if computed) ----------
if "OP-to-GP Conversion" in extras and isinstance(extras["OP-to-GP Conversion"], pd.Series):
    s = extras["OP-to-GP Conversion"].dropna()
    if not s.empty:
        x_num, lbls = as_period_axis(s.index)
        y = [float(v) if pd.notna(v) else np.nan for v in s.values]
        plot_line_with_regression(x_num, y, lbls, "OP-to-GP Conversion (with Regression)", "Operating/Gross Profit (ratio)")

if "Margin Spread" in extras and isinstance(extras["Margin Spread"], pd.Series):
    s = extras["Margin Spread"].dropna()
    if not s.empty:
        x_num, lbls = as_period_axis(s.index)
        y = [float(v) if pd.notna(v) else np.nan for v in s.values]
        plot_line_with_regression(x_num, y, lbls, "Margin Spread: Gross - Operating (with Regression)", "Spread (ratio)")

print("\n✅ Done. Live charts shown with linear regression overlays where possible.")
